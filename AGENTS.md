...
This repository builds a static Dutch WK 2026 cheat sheet/PWA. `generate.py` fetches TheSportsDB event data, computes group standings, and renders `template.html` into `index.html`. Keep reusable markup and placeholders in `template.html`; treat `index.html` as generated output. The generator substitutes nine placeholders in `template.html`: `{{DATE}}`, `{{GROUPS}}` (group-card HTML), `{{STATS}}` (the cijfers/topscorers HTML), and `{{FX_DATA}}`, `{{TL_DATA}}`, `{{QUAL_DATA}}`, `{{THIRD_DATA}}`, `{{WINNERS_DATA}}`, `{{KO_SCORES}}` (inline JS data shared by the Speelkalender and the knockout bracket: fixtures/scores, per-match goals/cards, group winners/runners-up, best-third slot assignments, knockout winners/losers `W##`/`V##`, and knockout scores). Adding or renaming a placeholder requires matching changes in both files. Root PNG files (`icon-192.png`, `icon-512.png`, `icon-maskable-512.png`, `apple-touch-icon.png`) and `manifest.json` provide PWA assets. `.github/workflows/update.yml` runs the scheduled regeneration job (every 30 minutes via `*/30 * * * *`, plus manual `workflow_dispatch`) and commits `index.html` and `data.json` when either changed.
...
`thirds_table.json` is the static, committed official FIFA Annex C table (495 combinations) mapping which best third-placed teams fill which Round-of-32 group-winner slots (`1A,1B,1D,1E,1G,1I,1K,1L`). `render_bracket_data` emits `{{QUAL_DATA}}` (1X/2X from 2 matchdays on; the 3rd element of each `QUAL` entry = confirmed?, so provisional teams render greyed), `{{THIRD_DATA}}` (best-third assignments once the whole group stage ends; never shown provisionally), and `{{WINNERS_DATA}}`/`{{KO_SCORES}}` (knockout winners `W##`, losers `V##`, and scores, propagated as each duel finishes). The knockout section itself is built entirely client-side: `renderBracket()` in `template.html` assembles the bracket tree of team cards from the `KO` fixtures array via `koTeam()` + `KOSCORE`/`KOWIN` — the same data the Speelkalender consumes. Connectors span exactly between each match's two feeder duels (vertical join from feeder-centre to feeder-centre), and tapping/hovering a duel traces the two games that feed it (`.bm.focus`/`.bm.feed`). There is no longer a static bracket DOM or tap popover.
