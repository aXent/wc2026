...
`thirds_table.json` is the static, committed official FIFA Annex C table (495 combinations) mapping which best third-placed teams fill which Round-of-32 group-winner slots (`1A,1B,1D,1E,1G,1I,1K,1L`). `render_bracket_data` fills `1X`/`2X` slots once a group is complete and the best-third slots once the whole group stage ends; the template's bracket-fill script applies `{{QUAL_DATA}}`/`{{THIRD_DATA}}` to the static bracket DOM client-side. Knockout winner propagation (`W##`) is not yet implemented.
